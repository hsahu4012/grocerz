...
const Subcategory = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState(subcategory_id || null);
  ...
  useEffect(() => {
    const fetchSubcategory = async () => {
      if (subcategory_id) {
        try {
          const response = await axios.get(`${process.env.REACT_APP_API_URL}subCategory/subCategoryById/${subcategory_id}`);
  ...
  const fetchProducts = async (categoryId) => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}products/bySubCategoryId/${categoryId}`);
                      ...
                      <ul className="sidebar-list">
                        {categories.map(category => (
                          <li key={category.category_id}>
                            ...
                            />
                            <label htmlFor={category.category_id}>{category.categoryname}</label>
                          </li>
                        ))}
                      </ul>
                    </div>
                    ...
                    <div className="col-xl-4 col-sm-6 mb-4" key={product.productid}>
                      <div className="product-wrapper" data-aos="fade-up">
                        ...
                        <div className="product-content" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
                          <h1 className="product-title" style={{ margin: '5px 0' }}>{product.prod_name}</h1>
                          <p className="product-price" style={{ margin: '5px 0' }}>${product.price}</p>
                          <button onClick={() => handleAddToCart(product.productid)} className="btn btn-primary" style={{ margin: '10px 0' }}>Add to Cart</button>
                          <button onClick={() => addToWishlist(product.productid)} className="btn btn-secondary" style={{ margin: '10px 0' }}>Add to Wishlist</button>
                        </div>
                      </div>
                    </div>
                  ))
